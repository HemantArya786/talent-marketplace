...
const ManualFormPage = () => {
  ...
  const handleChange =
    (field: string, index: number | null = null) =>
  ...
  const handleProjectThumbnailChange =
    (index: number) => (e: React.ChangeEvent<HTMLInputElement>) => {
      const file = e.target.files?.[0] || null;
      const updated = [...formData.projects];
      updated[index].thumbnail = file;
      updated[index].thumbnailPreview = file ? URL.createObjectURL(file) : null;
      setFormData((prev) => ({ ...prev, projects: updated }));
    };
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="bg-card text-foreground shadow-xl rounded-3xl w-full max-w-4xl p-10 animate-in fade-in duration-700 overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {["skills", "portfolioLinks"].map((field) => (
            <div key={field}>
          ...
          <div>
            ...
            {formData.projects.map((project, index) => (
              ...
              >
                ...
                <input
                  type="url"
                  placeholder="Project Link (e.g., GitHub or Live URL)"
                  value={project.link}
          ...
          {["accomplishments", "extracurricular"].map((field) => (
            <div key={field}>
              <label className="block mb-1 text-sm font-medium">
                {field
                  .replace(/([A-Z])/g, " $1")
