...
const CompanyFormPage = () => {
  const navigate = useNavigate();
  const { clientId } = useParams(); // get clientId if editing
  ...
  useEffect(() => {
    if (!clientId) return; // only fetch if editing
    ...
    const fetchClient = async () => {
      try {
        ...
        if (res.ok) {
          // Make sure clientDetails is always an array
          setFormData({
            name: data.name || "",
            email: data.email || "",
            phone: data.phone || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(`Form ${clientId ? "updated" : "submitted"} successfully!`);
      if (!clientId) navigate(`/company/portfolio/${data.clientId}`);
